  ...
  callbacks: {
    async jwt({ token, user, account, trigger, session }) {
      ...
      if (trigger === 'update' && session?.role) {
        ...
        // Mark role as confirmed when explicitly set via session update
        token.roleConfirmed = true;
        console.log('JWT updated with new role:', token);
      }
    ...
    async redirect({ url, baseUrl }) {
      ...
      if (url.startsWith(baseUrl) && url.includes('/auth/signin-callback')) {
        ...
        try {
          ...
          if (emailParam) {
            ...
            if (user && !user.roleConfirmed) {
              ...
              return `${baseUrl}/auth/role-select`;
            }
          }
        } catch (error) {
